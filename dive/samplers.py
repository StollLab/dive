...
def randP_p(delta,tauKtK,tauKtS,LtL,nr):
    r"""
    The pymc3 compatible version of randP, based on:
    J.M. Bardsley, C. Fox, An MCMC method for uncertainty quantification in
    nonnegativity constrained inverse problems, Inverse Probl. Sci. Eng. 20 (2012)
    """
    invSigma = tauKtK + delta*LtL
    ...
    v = np.random.standard_normal(size=(nr,))
    # w = tnp.lstsq()(T.transpose(C_L),v,rcond=None)
    w = snp.solve(T.transpose(C_L),v)
    # w = w[0]

    P = fnnls_T(invSigma,tauKtS+w,nr)

    return P     


def fnnls_T(AtA,Atb,nr,tol=[],maxiter=[],verbose=False):
#=====================================================================================
    r"""
    ...
    x[~passive] = 0
    w = Atb - pm.math.dot(AtA,x)     # negative gradient of error functional 0.5*||A*x-y||^2
    
    # Outer loop: Add variables to positive set if w indicates that fit can be improved.
    outIteration = 0
    maxIterations = 5*N    
    while T.any(pm.math.lt(w,tol)) and np.any(~passive):
        outIteration += 1
