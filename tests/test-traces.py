...
def test_running_regularization(t,Vexp):
    ...
    trace = dive.sample(model,draws=100,tune=200,chains=1,random_seed=100)
    lamb = trace.posterior.lamb[0][0].values
    assert lamb == 0.39950062979541884
...
def test_running_gaussian_1(t,Vexp):
    ...
    trace = dive.sample(model,draws=100,tune=200,chains=1,random_seed=100)
    lamb = trace.posterior.lamb[0][0].values
    assert lamb == 0.4011008024924596
...
def test_running_gaussian_2(t,Vexp):
    ...
    trace = dive.sample(model,draws=100,tune=200,chains=1,random_seed=100)
    lamb = trace.posterior.lamb[0][0].values
    assert lamb == 0.39991417362040915